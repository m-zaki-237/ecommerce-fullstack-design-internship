...
const CartPage = () => {
  ...
  const {
    cart, loading, fetchCart, updateQty, removeItem,
  ...
  const total = Math.max(0, parseFloat((subtotal + shipping + tax - discount).toFixed(2)));

  const handleApplyCoupon = async () => {
    if (!couponInput.trim()) return;
    try {
      const saved = await applyCoupon(couponInput.trim());
      setCouponMsg(`✅ Coupon applied! You saved $${Number(saved).toFixed(2)}`);
    } catch (err) {
                ...
                <div key={item._id} className={`flex gap-4 p-4 ${i < items.length - 1 ? "border-b border-gray-100" : ""}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button onClick={() => saveForLater(item._id)} className="text-xs text-blue-500 hover:underline transition-colors">Save for later</button>
                    </div>
                  </div>
                  ...
                  <div key={item._id} className="card overflow-hidden hover:shadow-md transition-shadow">
                    ...
                    <div className="p-3">
                      ...
                      <div className="flex gap-1">
                        <button onClick={() => moveToCart(item._id)} className="flex-1 border border-blue-500 text-blue-500 rounded-lg py-1.5 text-xs font-medium hover:bg-blue-50 transition-colors">
                          Move to cart
                        </button>
        ...
        <div className="md:w-72 shrink-0">
          <div className="card p-5 sticky top-24">
            ...
            <div className="border-t border-gray-100 pt-4 space-y-2.5">
              ...
              {discount > 0 && (
                <div className="flex justify-between text-sm font-medium text-green-600 bg-green-50 px-2 py-1.5 rounded-lg">
